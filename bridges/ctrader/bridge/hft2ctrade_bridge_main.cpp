...
int hft2ctrade_bridge_main(int argc, char *argv[])
{
    ...
    hft2ctrader_bridge_config cfg { hft2ctraderOption(config_file_name), hft2ctraderOption(broker), hft2ctraderOption(hft_ipc_host), hft2ctraderOption(hft_ipc_port) };
    ...
    logger_cfg.parseFromText(cfg.get_logging_config(hft2ctraderOption(log_severity)).c_str());
    ...
    try
    {
        ...
        if (cfg.get_account_type() == hft2ctrader_bridge_config::account_type::DEMO_ACCOUNT)
        {
            hft2ctrader_log(INFO) << "Account type: DEMO";
        }
        else if (cfg.get_account_type() == hft2ctrader_bridge_config::account_type::LIVE_ACCOUNT)
        {
    ...
    catch (const std::exception &e)
    {
        ...
        hft2ctrader_log(FATAL) << "**** FATAL ERROR: " << e.what();
